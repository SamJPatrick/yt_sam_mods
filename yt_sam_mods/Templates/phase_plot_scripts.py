...
def plot_number_density_temperature(profile_dir):
    
    xfield = 'number_density'
    yfield = 'temperture'
    input_filename = os.path.join(profile_dir, "DD0295_profile_temperature_number_density.h5")
    output_filename = os.path.join(profile_dir, "t_vs_n_vs_M_phase_plot.png")
    ...
    field = 'cell_mass'
    units = 'Msun'
    cmap = pyplot.get_cmap('dusk')
    #cmap = yt_colormaps['turbo']
    clabel = "M [M$_{\odot}$]"

    xlim = (1e-4, 1e9)
    xmajor = np.logspace(-4, 9, 8)
    xminor = np.logspace(-4, 9, 16)
    xlabel = "n [cm$^{-3}$]"

    ylim = (1e1, 2e3)
    ymajor = np.logspace(1, 1e3, 3)
    yminor = np.logspace(1, 1e3, 30)
    ylabel = "T [K]$"
...
if __name__ == "__main__":
    ...
    plot_number_density_temperature("Profiles")
